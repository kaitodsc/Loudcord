...
void DiscordVoiceClient::HandleGatewaySessionDescription(const VoiceGatewayMessage& m) {
    ...
    msg.Delay = 0;
    msg.SSRC = m_ssrc;
    msg.Speaking = VoiceSpeakingType::Priority;
    m_ws.Send(msg);
...
void ModifyAudioSamples(std::vector<uint8_t>& audioSamples, float volumeScale) {
    // Each sample is 64-bit (8 bytes) in this example
    const int bytesPerSample = 8;
    int numSamples = audioSamples.size() / bytesPerSample;

    for (int i = 0; i < numSamples; ++i) {
        // Convert the sample bytes to a signed 64-bit integer
        int64_t sample = static_cast<int64_t>((static_cast<int64_t>(audioSamples[i * bytesPerSample + 7]) << 56) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 6]) << 48) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 5]) << 40) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 4]) << 32) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 3]) << 24) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 2]) << 16) |
            (static_cast<int64_t>(audioSamples[i * bytesPerSample + 1]) << 8) |
            static_cast<int64_t>(audioSamples[i * bytesPerSample]));

        // Apply the volume scaling factor
        sample = static_cast<int64_t>(sample * volumeScale);

        // Clamp the sample to ensure it remains within the valid range of a 64-bit signed integer
        if (sample > INT64_MAX) {
            sample = INT64_MAX;
        }
        else if (sample < INT64_MIN) {
            sample = INT64_MIN;
        }

        // Convert the signed 64-bit integer back to sample bytes
        audioSamples[i * bytesPerSample] = static_cast<uint8_t>(sample & 0xFF);
        audioSamples[i * bytesPerSample + 1] = static_cast<uint8_t>((sample >> 8) & 0xFF);
        audioSamples[i * bytesPerSample + 2] = static_cast<uint8_t>((sample >> 16) & 0xFF);
        audioSamples[i * bytesPerSample + 3] = static_cast<uint8_t>((sample >> 24) & 0xFF);
        audioSamples[i * bytesPerSample + 4] = static_cast<uint8_t>((sample >> 32) & 0xFF);
        audioSamples[i * bytesPerSample + 5] = static_cast<uint8_t>((sample >> 40) & 0xFF);
        audioSamples[i * bytesPerSample + 6] = static_cast<uint8_t>((sample >> 48) & 0xFF);
        audioSamples[i * bytesPerSample + 7] = static_cast<uint8_t>((sample >> 56) & 0xFF);
