...
AudioManager::AudioManager() {
    ...
    m_capture_config.periodSizeInFrames = 1920;
...
void AudioManager::SetBandwidth(int bandwidth) {
    std::lock_guard<std::mutex> _(m_enc_mutex);
    if (int err = opus_encoder_ctl(m_encoder, OPUS_SET_BANDWIDTH(bandwidth)); err != OPUS_OK) {
        spdlog::get("audio")->error("opus_encoder_ctl(OPUS_SET_BANDWIDTH) failed: {}", err);
    }
}

void AudioManager::SetLSBDepth(int depth) {
    std::lock_guard<std::mutex> _(m_enc_mutex);
    if (int err = opus_encoder_ctl(m_encoder, OPUS_SET_LSB_DEPTH(depth)); err != OPUS_OK) {
        spdlog::get("audio")->error("opus_encoder_ctl(OPUS_SET_LSB_DEPTH) failed: {}", err);
    }
}

void AudioManager::SetForceChannels(int channels) {
    std::lock_guard<std::mutex> _(m_enc_mutex);
    if (int err = opus_encoder_ctl(m_encoder, OPUS_SET_FORCE_CHANNELS(channels)); err != OPUS_OK) {
        spdlog::get("audio")->error("opus_encoder_ctl(OPUS_SET_FORCE_CHANNELS) failed: {}", err);
    }
}
