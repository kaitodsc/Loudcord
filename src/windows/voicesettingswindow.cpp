...
VoiceSettingsWindow::VoiceSettingsWindow()
    ...
    m_signal.signal_changed().connect([this]() {
        ...
        // Set the bandwidth to OPUS_BANDWIDTH_FULLBAND for "Voice" and "Auto" signals
        if (signal == "Voice" || signal == "Auto") {
            audio.SetSignalHint(OPUS_SIGNAL_VOICE);
            audio.SetBandwidth(OPUS_BANDWIDTH_FULLBAND);
            audio.SetLSBDepth(16);
            audio.SetForceChannels(2);
    ...
    constexpr static auto MAX_BITRATE = 1280000.0;
